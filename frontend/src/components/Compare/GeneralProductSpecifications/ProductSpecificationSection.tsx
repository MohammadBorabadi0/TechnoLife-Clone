  ...
  return (
    <>
      {compareProducts.some((product) => product.specifications[dataKey]) && (
        <section className="flex flex-col gap-2 p-7 even:bg-[#f4f4f4] odd:bg-white rounded-lg mx-5 min-w-[335px]">
        {/* <section className="flex flex-col gap-2 p-7 even:bg-[#f4f4f4] odd:bg-white rounded-lg mx-5 min-w-[235px]"> */}
          <h3 className="text-base font-semibold leading-7.5 text-gray-800">
            {title}
          </h3>
          <div className="grid w-full gap-x-5 overflow-hidden 2xl:gap-x-9 grid-cols-4 text-gray-500 text-sm leading-7">
            {compareProducts.map(
              (product) =>
